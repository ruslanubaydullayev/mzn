...
function showDate(epochMS) {
  ...
  if (sameDay(today, instant)) {
    return this.$i18n.t('generic.today');
  }

  const yesterday = today.subtract(1, 'day');
  if (sameDay(yesterday, instant)) {
    return this.$i18n.t('generic.yesterday');
  }
...
function showDateTime(epochMS) {
  ...
  const today = dayjs();
  if (sameDay(today, instant)) {
    return `${this.$i18n.t('generic.today')} ${instant.format(TIME_FORMAT)}`;
  }

  const yesterday = today.subtract(1, 'day');
  if (sameDay(yesterday, instant)) {
    return `${this.$i18n.t('generic.yesterday')} ${instant.format(TIME_FORMAT)}`;
  }
